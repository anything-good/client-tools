...
-----END CERTIFICATE-----`],qB=GB,eA=$,PB=OB,_B=()=>qB.rootCertificates||PB,uA=w=>!!_B().find(I=>{try{const C=eA.pki.certificateFromPem(I);return eA.pki.certificateToPem(w)===I||C.issued(w)}catch{return!1}}),yA=w=>!!w.find((I,C)=>w[C+1]&&w[C+1].issued(I)),$B=w=>!!w.find(({validity:{notAfter:I,notBefore:C}})=>I.getTime()<Date.now()||C.getTime()>Date.now()),An=w=>yA(w)&&uA(w[w.length-1]);var Bn={authenticateSignature:An,verifyCaBundle:yA,verifyRootCert:uA,isCertsExpired:$B};const nn=JB,En=KB,Mn=sA,Qn=Bn;var jA={extractSignature:nn,...En,...Mn,...Qn};const In=$,{extractSignature:wn,getMessageFromSignature:Dn,preparePDF:gn}=jA,hA=w=>w.reduce((I,{name:C,value:V})=>(C&&(I[C]=V),I),{}),Cn=w=>{const{issuer:I,subject:C,validity:V}=w;return{issuedBy:hA(I.attributes),issuedTo:hA(C.attributes),validityPeriod:V,pemCertificate:In.pki.certificateToPem(w)}},zA=w=>w.map(Cn).map((I,C)=>C?I:{clientCertificate:!0,...I}),Nn=w=>{const I=gn(w),{signatureStr:C}=wn(I);return C.map(V=>{const{certificates:T}=Dn(V);return zA(T)})};var HA={extractCertificatesDetails:zA,getCertificatesInfoFromPDF:Nn};const x=$,aA=DA,{extractSignature:Gn,getMessageFromSignature:Vn,getClientCertificate:Tn,checkForSubFilter:Fn,preparePDF:rn,authenticateSignature:tn,sortCertificateChain:Un,isCertsExpired:cn}=jA,{extractCertificatesDetails:Yn}=HA,Rn=(w,I,C)=>{const V=Vn(w),{certificates:T,rawCapture:{signature:G,authenticatedAttributes:N,digestAlgorithm:Q}}=V,F=x.asn1.derToOid(Q),R=x.pki.oids[F].toLowerCase(),r=x.asn1.create(x.asn1.Class.UNIVERSAL,x.asn1.Type.SET,!0,N),i=Tn(T),o=x.md[R].create().update(x.asn1.toDer(r).data).digest().getBytes();if(!i.publicKey.verify(o,G))throw new aA("Wrong authenticated attributes",aA.VERIFY_SIGNATURE);const y=x.pki.oids.messageDigest,S=N.find(BA=>x.asn1.derToOid(BA.value[0].value)===y).value[1].value[0].value,k=x.md[R].create().update(I.toString("latin1")).digest().getBytes()===S,X=Un(T),s=Yn(X),v=tn(X),f=cn(X);return{verified:k&&v&&!f,authenticity:v,integrity:k,expired:f,meta:{certs:s,signatureMeta:C}}};var ln=w=>{const I=rn(w);Fn(I);try{const{signatureStr:C,signedData:V,signatureMeta:T}=Gn(I),G=V.map((N,Q)=>Rn(C[Q],N,T[Q]));return{verified:G.every(N=>N.verified===!0),authenticity:G.every(N=>N.authenticity===!0),integrity:G.every(N=>N.integrity===!0),expired:G.some(N=>N.expired===!0),signatures:G}}catch(C){return{verified:!1,message:C.message,error:C}}};const mA=ln,{getCertificatesInfoFromPDF:en}=HA;Object.assign(mA,{getCertificatesInfoFromPDF:en});var hn=mA;const an=MB(hn),on={style:{flex:"0 0 100%"}},dn={"mx-auto":"","max-w-600px":""},xn={"font-bold":""},bn={key:1},sn={key:0,style:{flex:"0 0 100%"},"mt-5":"",flex:"","flex-col":"","gap-4":""},Zn={"mb-2":"","font-bold":""},Jn=oA({__name:"pdf-signature-checker",setup(w){const I=MA([]),C=MA("idle"),V=MA(null);async function T(G){V.value=G;const N=await G.arrayBuffer();C.value="loading";try{const{signatures:Q}=an(N);I.value=Q,C.value="parsed"}catch{I.value=[],C.value="error"}}return(G,N)=>{const Q=DB,F=gB,R=CB,r=rB;return z(),J(iA,null,[W("div",on,[W("div",dn,[j(Q,{title:"Drag and drop a PDF file here, or click to select a file",accept:".pdf",onFileUpload:T}),u(V)?(z(),QB(F,{key:0,"mt-4":"",flex:"","gap-2":""},{default:H(()=>[W("div",xn,P(u(V).name),1),W("div",null,P(u(VB)(u(V).size)),1)]),_:1})):QA("",!0),u(C)==="error"?(z(),J("div",bn,[j(R,{"mt-4":""},{default:H(()=>[IB(" No signatures found in the provided file. ")]),_:1})])):QA("",!0)])]),u(C)==="parsed"&&u(I).length?(z(),J("div",sn,[(z(!0),J(iA,null,wB(u(I),(i,o)=>(z(),J("div",{key:o},[W("div",Zn," Signature "+P(o+1)+" certificates : ",1),j(r,{signature:i},null,8,["signature"])]))),128))])):QA("",!0)],64)}}});export{Jn as default};
