...
(function (X) {
	...
	r.prototype.Graph = function (c, d) {
		function a(q, M) {
			for (n = f.maxValue = 0; n < f.values.length; n++)
				f.values[n] > f.maxValue && (f.maxValue = f.values[n]);
			...
			if (1 < f.values.length) {
				...
				for (n = 0; n < f.values.length; n++) {
					...
					Q =
						((130 / (f.values.length - 1)) * n).toFixed(2) +
						"," +
						(f.height - f.height * Q).toFixed(2) +
						" ";
					F += Q;
				}
				F += "130," + f.height;
			...
			for (n = 0; n < f.vSteps; n++)
				f.measurements.push(Math.ceil((f.maxValue / f.vSteps) * (n + 1)));
			f.measurements.reverse();
			for (F = document.getElementsByClassName(w); 0 < F.length; )
				F[0].remove();
			f.polygon = document.createElementNS(
		...
		var a = this,
			...
			t = setInterval(function () {
				var w = (Date.now() - f) / 1e3,
					...
					q = Oa(w, 400, -400, d);
				c
					? ((a.progressStatus_Desk.el.style.strokeDashoffset = n),
						(a.progressStatus_Mob.el.style.strokeDashoffset = n))
					: ((a.progressStatus_Desk.el.style.strokeDashoffset = q),
						(a.progressStatus_Mob.el.style.strokeDashoffset = q));
				w >= d &&
					(clearInterval(t),
					(Ma = "done"),
					(a.progressStatus_Desk.el.style.strokeDashoffset = 800),
					(a.progressStatus_Mob.el.style.strokeDashoffset = 800));
			}, 14);
	};
	r.prototype.mainGaugeProgress = function (c) {
		...
		0 < c &&
			...
			(this.mainGaugeBlue_Desk.el.style.strokeDashoffset = a),
			(this.mainGaugeWhite_Desk.el.style.strokeDashoffset = 0 == a ? 1 : a + 1),
			(this.mainGaugeBlue_Mob.el.style.strokeDashoffset = a),
			(this.mainGaugeWhite_Mob.el.style.strokeDashoffset = 0 == a ? 1 : a + 1));
		0 == a && 1e3 < d
			? ((this.mainGaugeBlue_Mob.el.style.strokeDashoffset =
					681 <= a ? 681 : a),
				(this.mainGaugeWhite_Mob.el.style.strokeDashoffset =
					0 == a ? 1 : a + 1),
				(this.mainGaugeWhite_Desk.el.style.strokeDashoffset =
					0 == a ? 1 : a + 1),
				(this.mainGaugeBlue_Desk.el.style.strokeDashoffset =
					681 <= a ? 681 : a))
			: 0 == a &&
				0 >= d &&
	...
	r.prototype.uploadResult = function (c) {
		...
		1e4 <= c &&
			99999 > c &&
			((this.upRestxt.el.textContent = c.toFixed(1)),
	...
	r.prototype.downloadResult = function (c) {
		...
		1e4 <= c &&
			99999 > c &&
			((this.downResult.el.textContent = c.toFixed(1)),
		...
		"Jitter" === d &&
			(1 <= a && 1e4 > a
				? ((this.jitterDesk.el.textContent = Math.floor(a)),
					...
					100 <= a &&
						((a = (a / 1e3).toFixed(1)),
						(this.JitterResultMon.el.textContent = a + "k")))
				: 0 <= a &&
					1 > a &&
					(0 == a && (a = 0),
			...
			"speedToZero" === d
				...
				: "Ping" === d
					? 1 <= a && 1e4 > a
						...
						: 0 <= a &&
							1 > a &&
							(0 == a && (a = 0), (this.oDoLiveSpeed.el.textContent = a))
					: (0 == a &&
							((f = a.toFixed(0)), (this.oDoLiveSpeed.el.textContent = f)),
						1 >= a &&
							0 < a &&
							((f = a.toFixed(3)), (this.oDoLiveSpeed.el.textContent = f)),
						1 < a &&
							((f = a.toFixed(1)), (this.oDoLiveSpeed.el.textContent = f)),
						1e3 >= a &&
							((this.oDoTopSpeed.el.textContent = "1000+"),
							(this.oDoTopSpeed.el.style.fontSize = "16.9px"),
							(this.oDoTopSpeed.el.style.fill = "gray")),
						1010 <= a &&
							((this.oDoTopSpeed.el.textContent =
								1e3 * Math.floor(a / 1010) + "+"),
							(this.oDoTopSpeed.el.style.fill = "gray"),
		...
		if (0 <= c)
			var f = Date.now(),
				...
				w = setInterval(function () {
					...
					if (3 <= n || 0 >= q)
						clearInterval(w),
							a.LiveSpeed(0, "speedToZero"),
							a.mainGaugeProgress(0),
							(m = d);
				}, 16);
	};
	r.prototype.getNonlinearDegree = function (c) {
		...
		for (; d < this.scale.length; )
			if (c > this.scale[d].value) d++;
				...
				return (
					this.scale[d - 1].degree +
					((c - this.scale[d - 1].value) *
						(this.scale[d].degree - this.scale[d - 1].degree)) /
						(this.scale[d].value - this.scale[d - 1].value)
				);
	...
	O.prototype.AvgSpeed = function (c, d, a) {
		...
		this.FinalSpeed;
		this.timeNow >= a - d &&
			(0 < c && this.SpeedSamples.push(c),
			(this.FinalSpeed =
				this.ArraySum(this.SpeedSamples) / this.SpeedSamples.length));
		return this.FinalSpeed;
	};
	O.prototype.uRandom = function (c, d) {
		for (var a = new Uint32Array(262144), f = [], t = Blob, w = 0; w < c; w++) {
			for (var n = w, q = a.length, M = 0; M < q; M++)
				a[M] = 4294967296 * Math.random();
			f[n] = a;
		}
		return new t(f, { type: "application/octet-stream" }, Na(d));
	};
	...
	var jb = function () {
		...
		function d() {
			function b() {
				1 <= l
					? (--l, e.LiveSpeed(l, "countDown"))
					: 0 >= l && (clearInterval(g), (G = !0), (Z = void 0), d());
			}
			Pa &&
				(x.remEvt(e.settingsMob.el, "click", c),
				x.remEvt(e.settingsDesk.el, "click", c),
				x.remEvt(e.startButtonDesk.el, "click", d),
				x.remEvt(e.startButtonMob.el, "click", d),
				x.remEvt(document, "keypress", a),
				(Pa = !1));
			if (0 <= Z) {
				G = !1;
				e.userInterface();
			...
			"fetch" === openSpeedTestServerList &&
				!0 === G &&
				((G = !1), e.showStatus("Fetching Server Info.."), Y(6));
			!0 === G &&
				("Ping" === A
					? f()
					: "Download" === A
		...
		function t() {
			...
			("widget" !== openChannel && "web" !== openChannel) || Y(1);
			var l = setInterval(function () {
				"Loaded" === m && ((m = "busy"), Q());
				"Ping" === m && ((m = "busy"), e.showStatus("Milliseconds"));
				"Download" === m &&
					(e.showStatus("Initializing.."),
					x.reset(),
					(y = ha = 0),
					e.reset(),
					(wa = window.performance.now()),
					w(),
					(m = "initDown"));
				if ("Downloading" === m) {
					e.Symbol(0);
					...
					xa >= dlDuration &&
						"done" == Ma &&
						(A
							? (e.GaugeProgresstoZero(y, "SendR"),
								e.showStatus("All done"),
								e.Symbol(2))
							: e.GaugeProgresstoZero(y, "Upload"),
						e.downloadResult(R),
						(ia = S),
						(I = 1),
						(m = "busy"),
				...
				"Upload" == m &&
					...
					(m = "initup"),
					e.showStatus("Initializing.."),
					e.LiveSpeed("...", "speedToZero"),
					(za = x.uRandom(ulDataSize, b)),
					A && (aa = 1));
				"Uploading" === m &&
					(1 == aa &&
						((aa = 2),
						e.showStatus("Testing upload speed.."),
						(y = 0),
						x.reset(),
						e.reset(),
						(Ra = g = (window.performance.now() - va) / 1e3),
						e.progress(!1, ulDuration + 2.5),
						(ulDuration += g)),
					e.showStatus("Mbps upload"),
					(T = (window.performance.now() - va) / 1e3),
					q("up"),
					e.mainGaugeProgress(y),
					e.LiveSpeed(y),
					e.Graph(y, 1),
					(U = x.AvgSpeed(y, Aa, ulDuration)),
					T >= ulDuration &&
						1 == I &&
						((ja = P),
						e.uploadResult(U),
				...
				if ("Error" === m) {
					...
					g.innerHTML =
						'<a xlink:href="https://openspeedtest.com/FAQ.php?ref=NetworkError" style="cursor: pointer" target="_blank"></a>';
					var u = g.querySelector("a");
					e.oDoLiveSpeed.el.textContent = "Network Error";
					var C = document.getElementById("oDoLiveSpeed");
				...
				"SendR" === m &&
					...
					(g = document.createElement("div")),
					(g.innerHTML =
						'<a xlink:href="https://openspeedtest.com?ref=Self-Hosted-Outro&run=5" style="cursor: pointer" target="_blank"></a>'),
					(u = g.querySelector("a")),
					(e.oDoLiveSpeed.el.textContent = fb),
					(C = document.getElementById("oDoLiveSpeed")),
					(u.innerHTML = C.innerHTML),
					(C.innerHTML = g.innerHTML),
					location.hostname != Ba.toLowerCase() + Sa
						? ((ba =
								"https://" +
								Ba.toLowerCase() +
								Sa +
								...
								Da +
								"&dd=" +
								(ia / 1048576).toFixed(3) +
								"&ud=" +
								(ja / 1048576).toFixed(3) +
								"&ua=" +
								Ta),
							(ba = encodeURI(ba)),
		...
		function q(b) {
			if ("dl" === b) {
				...
				l > 1e3 * Qa + (ya / 2) * 1e3 &&
					0 === Ua &&
					((Ua = 1), (ka *= 0.01), (ca *= 0.01), (la = l + 1e4));
				l >= la && la < gb && ((la += 1e4), (ka *= 0.01), (ca *= 0.01));
				Va = 0 >= S ? 0 : S - Wa;
				Wa = S;
				ca += Va;
				Xa = Ea = l - Ea;
			...
			"up" === b &&
				...
				b > 1e3 * Ra + (Aa / 2) * 1e3 &&
					0 === Ya &&
					((Ya = 1), (ma *= 0.1), (da *= 0.1), (na = b + 1e4)),
				b >= na && na < hb && ((na += 1e4), (ma *= 0.1), (da *= 0.1)),
				(Za = 0 >= P ? 0 : P - $a),
				($a = P),
				(da += Za),
				(ab = Fa = b - Fa),
		...
		function M(b) {
			...
			z[b].onprogress = function (g) {
				if (1 === I)
					return z[b].abort(), (z[b] = null), (z[b] = void 0), delete z[b], !1;
		...
		function F(b) {
			...
			v[b].upload.onprogress = function (g) {
				...
				if (T >= ulDuration)
					return v[b].abort(), (v[b] = null), (v[b] = void 0), delete v[b], !1;
				var u = 0 >= g.loaded ? 0 : g.loaded - l;
				if (isNaN(u) || !isFinite(u) || 0 > u) return !1;
				P += u;
				l = g.loaded;
			...
			v[b].onload = function () {
				if (0 === l && ((P += 1048576 * ulDataSize), T >= ulDuration))
					return v[b].abort(), (v[b] = null), (v[b] = void 0), delete v[b], !1;
			...
			function u() {
				...
				L.onload = function () {
					if (200 === this.status && 4 === this.readyState) {
						...
						B = B[B.length - 1];
						B =
							"xmlhttprequest" === B.initiatorType
								? parseFloat(B.duration.toFixed(1))
								: D;
						250 < C && (B = D);
						0 >= B ? J.push(0.1) : J.push(B);
						1 < J.length &&
							((D = Math.abs(J[J.length - 1] - J[J.length - 2]).toFixed(1)),
							K.push(parseFloat(D)),
							e.LiveSpeed(B, "Ping"),
							e.pingResults(B, "Ping"),
							e.jitterResult(D, "Jitter"));
						g();
					}
		...
		var za,
			...
			ba,
			I = 0;
		var Ta = window.navigator.userAgent
			? window.navigator.userAgent
			: "Not Found";
		var Aa = 0.6 * ulDuration,
			ya = 0.6 * dlDuration;
		7 <= 0.6 * ulDuration && (Aa = 7);
		7 <= 0.6 * dlDuration && (ya = 7);
		var G = !0,
			ua = !0;
		...
		if (
			setPingSamples &&
			("string" === typeof k.ping || "string" === typeof k.p)
		) {
		...
		if (
			setPingTimeout &&
			("string" === typeof k.out || "string" === typeof k.o)
		) {
		...
		if (setHTTPReq && ("string" === typeof k.xhr || "string" === typeof k.x)) {
			if ("undefined" !== typeof k.xhr) var ra = k.xhr;
			else "undefined" !== typeof k.x && (ra = k.x);
			0 < ra && 32 >= ra && (ulThreads = dlThreads = ra);
		}
		if (
			selectServer &&
			("string" === typeof k.host || "string" === typeof k.h)
		) {
			...
			/(?:https?):\/\/(\w+:?\w*)?(\S+)(:\d+)?(\/|\/([\w#!:.?+=&%!\-\/]))?/.test(
				...
				(openSpeedTestServerList = [
					{
						ServerName: "Home",
						Download: "https://link.testfile.org/60MB",
						Upload: sa + "/upload",
		...
		var Z;
		!enableRun ||
			("string" !== typeof k.run && "string" !== typeof k.r) ||
			(Z = 0 < p ? p : 0 < H ? H : 0);
		0 <= Z && G && d();
		p = k.test;
		k = k.t;
		...
		var Va = 0,
			...
			Ya = 0,
			la,
			gb = 1e3 * dlDuration - 6e3,
			na,
			hb = 1e3 * ulDuration - 6e3,
			v = [],
			Ga,
			qa,
			Ja,
			...
			Y = function (b) {
				...
				l.onreadystatechange = function () {
					4 == l.readyState &&
						...
						6 == b &&
							((openSpeedTestServerList = JSON.parse(V)), (G = !0), d()),
						7 == b && (e.YourIP.el.textContent = V));
				};
				2 == b && (ea = "r=n");
				3 == b &&
					(ea =
